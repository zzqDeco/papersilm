...
TUI and release-gate changes should run `scripts/tui-smoke.sh`. The script covers
snapshot-backed visual scenarios, PTY smoke frames, service/runtime flow, real
binary process-level TUI E2E, TUI packages, full tests, vet, build, and
`git diff --check`. When a PTY or process scenario fails, the test output includes
an artifact directory containing raw ANSI captures, normalized frames, rendered
screens, scenario metadata, and session summaries. Set
...
Workspace tool behavior is documented in [doc/workspace-tools.md](doc/workspace-tools.md). The stable Eino tool-call contract is that localized edits use `workspace_replace_text`, whole-file changes use `workspace_write_file`, `workspace_run_command` returns structured results for non-zero shell exits instead of failing the whole agent turn, and session allow rules preserve the request operation (`replace`, `write`, or `shell`).
