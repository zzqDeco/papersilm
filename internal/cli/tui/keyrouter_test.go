...
func TestRouteKeyApprovalSelectsBeforeChat(t *testing.T) {
	...
	action := RouteKey([]KeyContext{ContextConfirmation, ContextAutocomplete, ContextPane, ContextChat, ContextGlobal}, "enter")
	if action != ActionApprovalCommit {
		t.Fatalf("expected approval commit before chat submit, got %q", action)
	}

	action = RouteKey([]KeyContext{ContextConfirmation, ContextAutocomplete, ContextPane, ContextChat, ContextGlobal}, "down")
	if action != ActionApprovalNext {
		t.Fatalf("expected approval next before chat history, got %q", action)
	}

	action = RouteKey([]KeyContext{ContextConfirmation, ContextAutocomplete, ContextPane, ContextChat, ContextGlobal}, "tab")
	if action != ActionApprovalFeedback {
		t.Fatalf("expected tab to enter feedback mode, got %q", action)
	}

	action = RouteKey([]KeyContext{ContextConfirmation, ContextAutocomplete, ContextPane, ContextChat, ContextGlobal}, "ctrl+e")
