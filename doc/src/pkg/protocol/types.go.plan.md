...
## 2. 核心职责
- 定义整个系统的核心协议类型，包括来源、摘要、对比、DAG、执行状态、artifact、会话和请求/结果对象。
...
## 4. 关键实现细节
- 主要类型: `SourceType`、`SourceStatus`、`PermissionMode`、`OutputFormat`、`ArtifactFormat`、`ContentSource`、`WorkerProfile`、`NodeKind`、`NodeStatus`、`SourceInspection`、`PaperRef`、`Citation`、`KeyResult`、`PaperDigest`、`ComparisonMatrixRow`、`ComparisonDigest`、`PlanNode`、`PlanEdge`、`PlanDAG`、`NodeOutputRef`、`DagPatch`、`BatchStatus`、`ExecutionBatch`、`NodeExecutionState`、`ExecutionState`、`PlanStep`、`PlanResult`、`ApprovalRequest`、`PlanProgressStatus`、`PlanProgress`、`ArtifactManifest`、`SessionState`、`SessionMeta`、`SessionSnapshot`、`ClientRequest`、`RunResult`。
- 前半部分覆盖来源、权限模式、输出格式、worker profile、node kind/status 等基础枚举。
- 中段定义 `PaperDigest`、`ComparisonDigest`、`PlanDAG`、`ExecutionState` 等核心对象。
- 后半部分定义 `PlanResult`、`ApprovalRequest`、`PlanProgress`、`ArtifactManifest`、`SessionMeta`、`RunResult` 等高层协作结构。
