...
- 为审批场景、artifact 导出、会话资产查询和 workspace / skill 暴露提供统一的工具层入口。

## 3. 输入与输出
- 输入来源: `pipeline.Service`、会话存储、session ID、工具输入结构以及 tool runtime 上下文。
- 输出结果: `tool.BaseTool` 列表、来源列表、摘要/对比产物、artifact manifest 和包含 workspace / skill runs / skill artifacts 的会话资产集合。
...
- `list_session_assets` 会直接返回 `SessionSnapshot` 中已经 hydration 的 `workspaces`、`skill_runs` 与 `skill_artifacts`。
