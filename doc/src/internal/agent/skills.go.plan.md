...
- `builtinSkillDescriptors` 是 v1 的唯一技能来源；不做远程注册、插件发现或动态下载。
- `RunSkill()` 先读取当前 snapshot，再解析目标、持久化 running run record，随后同步执行 skill 并把结果写入独立 skill artifact 目录。
...
- skill artifact 沿用 `ArtifactManifest`，但走 `skill-artifacts/` 独立目录，因此 `InvalidatePlanState()` 不会误删 skill 结果。
