...
## 4. 关键实现细节
- 关键函数/方法: `RunTask`、`ApproveTask`、`RejectTask`、`rejectTask`、`runScopedExecution`、`finishTaskApproval`、`dependencyClosure`、`cascadeRerun`、`deleteArtifactsForNodes`。
- `RunTask()` 只继承当前已规划会话的语言/风格；如果调用方试图在 task 级覆写配置，会直接报错并要求先更新会话配置后重新 `/plan`。
...
- `RejectTask()` / `ApproveTask(..., false, ...)` 现在都执行真正的 task 级 reject：required task 记为 failed，optional task 记为 skipped，不再把整批 pending task 一起清空。
...
## 6. 变更影响面
- 这里直接定义 `/task run`、`/task approve`、`/task reject` 的用户语义，错误地放宽或收紧条件都会影响审批流和 rerun 行为。
